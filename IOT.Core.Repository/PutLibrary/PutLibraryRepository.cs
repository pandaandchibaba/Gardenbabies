...
namespace IOT.Core.Repository.PutLibrary
{
    public class PutLibraryRepository : IPutLibraryRepository
    {
        public int Delete(string ids)
        {
            ...
            DapperHelper.Execute($"INSERT INTO Lognote VALUES(NULL,'删除入库ID为{ids}的入库表信息',NOW(),'入库表')");
        ...
        public int Insert(Model.PutLibrary Model)
        {
            ...
            DapperHelper.Execute($"INSERT INTO Lognote VALUES(NULL,'添加入库单号为{Model.PutNO}的入库表信息',NOW(),'入库表')");
            return DapperHelper.Execute(sql);
        }
        ...
        public int Update(Model.PutLibrary Model)
        {
            ...
            DapperHelper.Execute($"INSERT INTO Lognote VALUES(NULL,'修改入库单号为{Model.PutNO}入库表信息',NOW(),'入库表')");
            return DapperHelper.Execute(sql);
        }
    }
}
